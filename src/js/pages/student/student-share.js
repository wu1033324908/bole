...
$(function () {
    ...
    initTable();
    // 表格搜索
    $.initSearchTable(_TABLE, _HOST.add_rort + _HOST.share.list);
    // 详细信息
    initModifyModule();
    // 点击表格上的图片,在浏览器显示大图
    ...
    function initTable() {
        _TABLE = $('#table').dataTable({
            ajax: {
                url: _HOST.add_rort + _HOST.share.list,
                type: 'POST',
                data: {
            ...
            columns: [{
                ...
                {
                    title: '推荐时间',
                    data: 'ReferrerCreatedAt',
                    className: 'show-detail-td',
                    render: (data, type, row, meta) => {
                        return `<div class="td-ellipsis" title="${data.substring(0, 19)}">${data.replace('T', ' ').substring(0, 19)}</div>`;
                    }
                }
            ],
