...
$(function () {
    ...
    initTable();
    // 表格搜索
    $.initSearchTable(_TABLE, _HOST.add_rort + _HOST.student.jigsaw.list_self);
    // 详细信息
    initModifyModule();
    ...
    function initTable() {
        _TABLE = $('#table').dataTable({
            ajax: {
                url: _HOST.add_rort + _HOST.student.jigsaw.list_self,
                type: 'POST',
                data: {
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: (data, type, row, meta) => {
                        if (data) {
                            return `<img class="show_resource_in_browser" title="${row.Url}" src="${data}" data-src="${row.Url}"/>`;
                        } else {
                            return '无';
                        }
                    }
    ...
    function initModifyModule() {
        ...
        function initForm(fun_data) {
            ...
            $img_box.append(`
                <div class="item">
                    <img class="origin-image show_resource_in_browser" src="${fun_data.modify.ThumbnailUrl}" data-src="${fun_data.modify.Url}" title="查看大图" alt="">
                </div>
            `);
        }
    }
